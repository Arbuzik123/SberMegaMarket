...
async def SBFind(cat):
    ...
    for idx, chunk in enumerate(chunks):
        path = rf"UpdatesSBFinds/" + cat.replace(".xlsx", "") + f"_{idx + 1}.xlsx"
    ...
    for i in range(num_chunks):
        p = Process(target=Sbermegasearch, args=(i, path, lock, window_PosX, window_PosY, positions.pop(0)))
        processes.append(p)
    ...
    output_path = os.path.join('UpdateResultSBFinds', rf'{cat.replace(".xlsx", "")}.xlsx')
